...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] int limiteAutmentos = 3;

    [SerializeField] private GameObject powerUpPrefab;
    [SerializeField] private GameObject powerDownPrefab;
    [SerializeField] private GameObject AnchoPrefab;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if (random >= 0.3f && random <= 0.45f)
            {
                Instantiate(AnchoPrefab, collision.transform.position, Quaternion.identity);
