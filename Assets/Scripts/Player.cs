...
public class Player : MonoBehaviour
{
    ...
    */
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Destroy(collision.gameObject); // Lo destruimos
            GameManager.Instance.LoseLifePower(); // Añadimos una vida
        }

        if (collision.CompareTag("PowerAncho")) // Si colisionamos con un powerUp
        {
            Destroy(collision.gameObject); // Lo destruimos
            GameManager.Instance.AnchoPower(gameObject); // Añadimos una vida
        }
    }
}
